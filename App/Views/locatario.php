<?php
    if ((isset($_SESSION)) && (isset($_SESSION['logado'])) && ($_SESSION['logado'] == "SIM")){
        extract($dataModel);
        extract($dadosLocatario);
        $dataModel = $listaImoveis;
    ?>
        <div class="container-fluid container-home bg-light text-primary">
            <h1 class="text-center braketop40 brakebottom20">Imóveis de <?= $NomeLocatarios; ?></h1>
            <p class="brakeleft80 brakeright80 text-dark text-center">Nome: <?= $NomeLocatarios; ?></p>
            <p class="brakeleft80 brakeright80 text-dark text-center">E-mail: <?= $EmailLocatarios; ?></p>
            <p class="brakeleft80 brakeright80 text-dark text-center">Telefone: <?= telefone($TelefoneLocatarios); ?></p>
